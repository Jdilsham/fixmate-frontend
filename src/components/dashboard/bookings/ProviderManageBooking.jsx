...
function StatusPill({ status }) {
  ...
  if (status === "IN_PROGRESS") {
    return (
      <span className={`${base} bg-yellow-500/15 text-yellow-400 border-yellow-500/20`}>
        IN PROGRESS
      </span>
    );
  }
  if (status === "PAYMENT_PENDING") {
    return (
      <span className={`${base} bg-emerald-500/15 text-emerald-400 border-emerald-500/20`}>
        PAYMENT PENDING
      </span>
    );
  }
...
export default function ProviderManageBooking({
  ...
  const paymentStatus = providerPaymentInfo?.paymentStatus ?? providerPaymentInfo?.status;
  ...
  return (
    // ✅ ONLY UI CHANGE: gradient is now flush at top like other cards
    <Card className="relative overflow-hidden rounded-3xl border bg-card shadow-sm">
      {/* Gradient line (flush to the top border) */}
