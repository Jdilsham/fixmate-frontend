...
export default function Dashboard() {
  ...
  const isPendingVerification = isProvider && verificationStatus === "PENDING";

  const isApproved = isProvider && verificationStatus === "APPROVED";
  ...
  const [customerManageBookingOpen, setCustomerManageBookingOpen] =
    useState(false);
    ...
    (async () => {
      try {
        ...
        const latest = updated.find(
          (b) => b.bookingId === managedBooking.bookingId,
        );
        if (latest)
          setManagedBooking((prev) => (prev ? { ...prev, ...latest } : prev));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await getProviderPaymentByBooking(
          managedBooking.bookingId,
        );
  ...
  const loadBookingsForDate = async (day) => {
    ...
    try {
      ...
      const filtered = allBookings.filter((b) => {
        ...
        const bookingDay = new Date(b.scheduledAt).toISOString().split("T")[0];
  ...
  const busyDates = new Set(
    providerBookings
      ...
      .map((b) => new Date(b.scheduledAt).toISOString().split("T")[0]),
  );
  ...
  const canToggleAvailability =
    role === "SERVICE_PROVIDER" && profile?.verificationStatus === "APPROVED";
  ...
  const handleRequestPayment = async (booking) => {
    try {
      ...
      try {
        const p = await getProviderPaymentByBooking(
  ...
  useRealTimeUpdates("booking-updates", loadBookings);
  ...
  return (
    <div className="relative min-h-screen text-foreground overflow-hidden">
      ...
      <div
        className="max-w-8xl mx-auto px-4 md:px-6 pt-6
      ...
      >
        <aside className="hidden lg:block w-64 shrink-0 h-full">
          <div
            className="bg-card/60 backdrop-blur-xl border border-white/10 dark:border-white/10
                    ...
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Pick a date to view bookings. Busy dates are marked
                          with a red dot.
                        </p>
                      </div>
                        ...
                        <div className="p-5 pt-6">
                          ...
                          <div className="rounded-2xl border bg-background/40 p-3 flex items-center justify-center">
                            <Calendar
                              ...
                              modifiersClassNames={{
                                busy: "relative after:absolute after:bottom-1 after:left-1/2 after:-translate-x-1/2 after:w-1.5 after:h-1.5 after:rounded-full after:bg-red-500",
                              }}
                          ...
                          {selectedDate &&
                            ...
                            (() => {
                              ...
                              if (calendarStatusFilter !== "ALL") {
                                list = list.filter(
                                  (b) => b.status === calendarStatusFilter,
                                );
                              }
                              ...
                              if (q) {
                                list = list.filter((b) => {
                                  ...
                                  return (
                                    name.includes(q) || service.includes(q)
                                  );
                                });
                              }
                        ...
                        <div className="px-6 py-5 border-b bg-card">
                          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                View requests, accept/reject pending ones, and
                    ...
                    <div className="flex flex-col gap-4 mb-4">
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            View your requests and complete payments when
                            required
                          </p>
                        </div>
                        ...
                        "
                          ...
                          <span className="px-3 py-2 rounded-full text-xs font-semibold bg-muted text-muted-foreground border border-border sm:self-center">
                            {filterCustomerBookings(customerBookings).length}{" "}
                            bookings
                          </span>
                        </div>
                      </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[
                            ...
                            },
                            {
                              label: "Accepted",
                              status: "ACCEPTED",
                              cls: "bg-green-500/15 text-green-500 border-green-500/20",
                            },
                            {
                              label: "Completed",
                              status: "COMPLETED",
                            ...
                            const count = (customerBookings || []).filter(
                              (b) => b.status === x.status,
                            ).length;
                            ...
                            return (
                              <button
                                key={x.status}
                                onClick={() =>
                                  setCustomerStatusFilter(x.status)
                                }
                                ...
                                {b.status === "PAYMENT_PENDING" && (
                                  <Button
                                    ...
                                    onClick={async () => {
                                      try {
                                        const payment =
                                          await getCustomerPayment(b.bookingId);
                ...
                {activeTab === "profile" && (
                  <>
                            ...
                            {role === "SERVICE_PROVIDER" &&
                              verificationStatus === "NOT_SUBMITTED" && (
                                <div className="rounded-2xl border bg-orange-500/5 p-4">
                                  ...
                                  <p className="text-xs text-muted-foreground mt-1">
                                    Upload ID + work proof to request
                                    verification and start receiving bookings.
                                  </p>
                        ...
                        >
                          {profile.verificationStatus === "PENDING" && (
                            <>
                              ...
                              <br />
                              Your documents are under admin review. You cannot
                              change availability or accept jobs until approval.
                            </>
                          )}
                          ...
                          {profile.verificationStatus === "APPROVED" && (
                            <>
                              ...
                              <br />
                              Your documents are approved. You can receive
                              bookings normally.
                            </>
                          )}
                          ...
                          {profile.verificationStatus === "NOT_SUBMITTED" && (
                            <>
                              ...
                              <br />
                              Please upload ID documents and work proof to
                    ...
                    <div className="mb-8">
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        Manage your personal details, documents, and security
                        settings.
                      </p>
                    </div>
                          ...
                          <Card className="rounded-3xl border bg-background/50 p-6 md:p-7 space-y-5 shadow-sm h-full">
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">
                                  Upload documents that prove your professional
                                  experience
                                </p>
                              </div>
                            ...
                            <div className="rounded-2xl border bg-card/50 p-4 space-y-3">
                              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setWorkPdf(e.target.files?.[0] || null)
                                  }
                              ...
                              {/* helper */}
                              <p className="text-xs text-muted-foreground">
                                Tip: upload a clear scanned document (letters,
                                certificates, work agreements, etc.)
                              </p>
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            await confirmBooking(
              ...
              acceptBooking.providerServiceId,
            );
            toast.success("Booking accepted");
